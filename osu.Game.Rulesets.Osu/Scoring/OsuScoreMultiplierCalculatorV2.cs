...
namespace osu.Game.Rulesets.Osu.Scoring
{
    public class OsuScoreMultiplierCalculatorV2 : ScoreMultiplierCalculator
    {
        ...
        // Torii: diverges from osu! here. osu! penalises ANY change to CS/HP/OD/AR symmetrically,
        // which is unfair for AR (lowering it is often an accessibility tweak for high-rate maps, and
        // lowering it a lot is actually *harder* to read), and doesn't punish the genuinely degenerate
        // CS0/OD0 plays hard enough. Instead: AR and HP never affect score; only *lowering* CS or OD is
        // penalised, by the absolute resulting value, so CS0 + OD0 bottoms out at the 0.1x floor.
        private static double difficultyAdjustMultiplier(OsuModDifficultyAdjust difficultyAdjust, IBeatmapDifficultyInfo beatmapDifficulty)
        {
            ...
            double csMultiplier = selectedCircleSize < beatmapDifficulty.CircleSize
                ? Math.Clamp(selectedCircleSize / 4.0, 0.2, 1.0)
                : 1.0;
            double odMultiplier = selectedOverallDifficulty < beatmapDifficulty.OverallDifficulty
                ? Math.Clamp(selectedOverallDifficulty / 5.0, 0.2, 1.0)
                : 1.0;

            return Math.Max(0.1, csMultiplier * odMultiplier);
        }
