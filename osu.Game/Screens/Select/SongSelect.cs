...
namespace osu.Game.Screens.Select
{
    public abstract partial class SongSelect : ScreenWithBeatmapBackground, IKeyBindingHandler<GlobalAction>, ISongSelect, IHandlePresentBeatmap, IProvideCursor
    {
        ...
        // each time. Against a private server the returned metadata frequently
        // differs from what the set was imported with, so that callback writes
        // realm on every selection, which fires the carousel's realm
        // subscription and re-filters the whole list on the update thread -> a
        // GC hiccup per map change. Caching the result for the session means a
        // given set is fetched (and its realm row touched) at most once, so
        // revisits are free and the churn stops after the first pass.
