        ...
        private void onStageChanged(RankedPlayStage stage)
        {
            ...
            switch (stage)
            {
                ...
                case RankedPlayStage.GameplayWarmup:
                    // GameplayWarmupScreen depends on the upstream
                    // BeatmapTitleWedge / BeatmapMetadataWedge refactor that
                    // pulls in the SongSelect rewrite (ISongSelect /
                    // BeatmapSetLookupResult), which conflicts heavily with
                    // Torii's parallel song-select forks. Per option 5a in
                    // RANKED_PLAY_PORT.md we skip the warmup phase entirely:
                    // when the server announces GameplayWarmup, fall straight
                    // through into the gameplay screen instead. Loses the
                    // warmup intro animation but keeps the play loop intact.
