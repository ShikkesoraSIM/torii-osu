    ...
    public partial class ToriiInterfaceSettings : SettingsSubsection
    {
        ...
        private void load(OsuConfigManager config)
        {
            Children = new Drawable[]
            {
                ...
                // Potato mode: read once at startup and gates heavy visuals
                // at construction, so it prompts for a restart on change
                // (same pattern as the UI-theme dropdown above). The control
                // owns its own checkbox + restart-confirm wiring.
